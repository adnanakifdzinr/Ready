...
// Brand Strategy: Smooth line-by-line slide reveal with stagger
export function BrandStrategyTextAnimation({ children, className = '' }: CustomAnimationProps) {
  const ref = useRef<HTMLDivElement>(null)
  const [isInView, setIsInView] = useState(false)
...
// Brand Clarity: Smooth fade with subtle upward motion
export function BrandClarityTextAnimation({ children, className = '' }: CustomAnimationProps) {
  const ref = useRef<HTMLDivElement>(null)
  const [isInView, setIsInView] = useState(false)
